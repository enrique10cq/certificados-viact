...
window.actualizarEmpleadoEdit = async function (idEmpleado) {
    try {
        ...
        if (response.status === 200) {
            ...
            <th class="dt-type-numeric sorting_1" scope="row">${infoEmpleado.id
                }</th>
            ...
            <td>
            <img class="rounded-circle" src="acciones/fotos_empleados/${infoEmpleado.avatar || "sin-foto.jpg"
                }" alt="${infoEmpleado.nombre}" width="50" height="50">
            </td>
            <td>
            <a title="Ver detalles del empleado" href="#" onclick="verDetallesEmpleado(${infoEmpleado.id
            })" class="btn btn-success"><i class="bi bi-binoculars"></i></a>
            <a title="Editar datos del empleado" href="#" onclick="editarEmpleado(${infoEmpleado.id
